...
- `evidence/demo-run.txt` - the full call demo sequence run end to end on 2026-09-19, including the live direct route
...
```bash
cp .env.example .env          # OFAPI_TOKEN is optional (only for --source=ofapi)
...
```bash
docker compose exec app ./vendor/bin/phpunit          # 96 PHP tests
docker run --rm -v "$PWD/services/rulegen:/app:ro" -w /app node:22-bookworm-slim \
  node --test test/rulegen.test.js                    # 22 rulegen tests
...
docker compose exec scheduler php artisan fans:onlyfans-rules --force   # extract + verify the current signer (the scheduler also does this within a minute)
docker compose exec app php artisan fans:demo live    # live call: direct OnlyFans route
docker compose exec scheduler php artisan fans:onlyfans-rules --status  # active build, canary, rejections
